#!/usr/bin/env python3
"""
Manages background tasks for table tennis picks - backed by tabletennis.py
(scores24.live), not scores365.py, which has no table tennis coverage at
all (see tabletennis.py's own module docstring). Two markets: "winner"
(moneyline) and "point_handicap" (combined match points, signed line) -
see tabletennis.grade_moneyline/grade_point_handicap.

No hibernation before kickoff (unlike tracker.py/settracker.py/etc) -
table tennis leagues run close to 24/7, so a match found by
tabletennis.find_match_for_team is always either already live or starting
within the hour; a plain poll-every-cycle loop never has a multi-hour idle
stretch worth hibernating through the way a scheduled game's kickoff does.
No auto-track retry queue either (mirrors esports.py/esportstracker.py's
own "found now or not tracked at all" choice, for the same reason - a
match this provider will ever have is already there by the time a pick
comes in).

Otherwise mirrors doublechancetracker.py/settracker.py: 🗑️-reaction
delete, restart-safe persistence, and a Won/Lost/Push/Voided result
reaction once graded.
"""
...
log = logging.getLogger("scorebox.tabletennistracker")
...
# Both markets are per-player - unlike, say, a combined game total, "team"
# genuinely disambiguates two different bets on the same match (e.g. a
# Point Handicap on either player is a different bet).
_PER_PLAYER_MARKETS = {"winner", "point_handicap"}
...
def list_tracked_details(channel_id: int) -> list[dict]:
    ...
    return [
        entry for entry in state.load_table_tennis().values()
...
def stop_tracking(channel_id: int, match_id, market: str, team: Optional[str] = None) -> bool:
    ...
    dailylog.record_result(channel_id, "tabletennistracker", key, "void", "Manually untracked")
...
def _grade(match: dict, market: str, team: str, line: Optional[float]) -> Optional[str]:
    if market == "point_handicap":
        return tabletennis.grade_point_handicap(match, team, line)
    return tabletennis.grade_moneyline(match, team)
...
async def build_embed(
    ...
    decided = tabletennis.is_finished(match)
    ...
    if force_result:
        ...
        color_status = result
    elif tabletennis.is_live(match):
    ...
    author_bits = [b for b in ("Table Tennis", tournament) if b]
    embed.set_author(name=" • ".join(author_bits))
    embed.description = pick_label(market, team, line)

    period_text = tabletennis.status_line(match)

    games = tabletennis.games_won(match)
    home_cols = [str(games[0])] if games else ["-"]
    away_cols = [str(games[1])] if games else ["-"]
    current = tabletennis.current_game_score(match)
    if current:
...
async def _track_loop(
    ...
    async def _repost_final(embed: discord.Embed, file: discord.File):
        ...
        try:
            ...
            log.warning("Failed to repost final table tennis tracking message: %s", e)
            try:
                await throttle.run(channel_id, lambda: message.edit(embed=embed, attachments=[file]))
            except discord.HTTPException as e2:
                log.warning("Failed to edit table tennis tracking message as a fallback: %s", e2)
            return carry_emojis
        try:
            await new_message.add_reaction(TRASH_EMOJI)
        except discord.HTTPException as e:
            log.warning("Failed to react to reposted final table tennis tracking message: %s", e)
        ...
        try:
            ...
            log.warning("Failed to delete old table tennis tracking message after final repost: %s", e)
        return carry_emojis

    async def _void_leg_and_give_up(reason: str):
        dailylog.record_result(channel_id, "tabletennistracker", key, "void", reason)
        group_ids = parlaytracker.groups_for_leg(channel_id, "tabletennistracker", key)
        if not group_ids:
            return
        ...
        await parlaytracker.handle_leg_result(
            message.channel, channel_id, message, "tabletennistracker", key,
    ...
    try:
        while time.monotonic() < deadline:
            ...
            updated = await asyncio.to_thread(tabletennis.get_live_update, current_match)
            if not updated:
                consecutive_misses += 1
                log.warning(
                    "Table tennis match %s not found on scores24.live (miss %d/%d)",
                    match_id, consecutive_misses, MAX_CONSECUTIVE_MISSES,
                )
                if consecutive_misses >= MAX_CONSECUTIVE_MISSES:
                    botlog.event(f"⚠️ Auto-stopped tracking (table tennis): match `{match_id}` not found {MAX_CONSECUTIVE_MISSES}x in a row, in <#{channel_id}>")
            ...
            if tabletennis.is_finished(current_match):
                ...
                if result:
                    dailylog.record_result(channel_id, "tabletennistracker", key, result)
                    group_ids = parlaytracker.groups_for_leg(channel_id, "tabletennistracker", key)
                    await parlaytracker.handle_leg_result(
                        message.channel, channel_id, message, "tabletennistracker", key, leg_label, result, group_ids,
                    )
                break

            detail = f"LIVE, {tabletennis.status_line(current_match)}" if tabletennis.is_live(current_match) else "NOT STARTED"
            dailylog.touch(channel_id, "tabletennistracker", key, detail)
            group_ids = parlaytracker.groups_for_leg(channel_id, "tabletennistracker", key)
            if group_ids:
                await parlaytracker.report_leg_progress(
                    message.channel, channel_id, message, "tabletennistracker", key, leg_label, detail, group_ids,
                )

            # Skips a redundant image re-upload (and the Discord edit call
            # entirely) when the rendered card is byte-identical to what's
            # already posted - same fix as boxingtracker.py's/settracker.
            # py's identical guard (this was tripping Discord's error
            # 400009).
            ...
            try:
                ...
                if e.status == 429 or e.code == 400009:
                    ...
                    log.warning(
                        "Failed to edit table tennis tracking message, rate limited (failure %d/%d): %s",
                    ...
                    if consecutive_rate_limit_failures >= MAX_CONSECUTIVE_RATE_LIMIT_FAILURES:
                        botlog.event(f"⚠️ Auto-stopped tracking (table tennis): match `{match_id}` message edit rate-limited {MAX_CONSECUTIVE_RATE_LIMIT_FAILURES}x in a row, in <#{channel_id}>")
                        await _void_leg_and_give_up("Message edit rate-limited repeatedly")
                        break
                    continue
                consecutive_edit_failures += 1
                log.warning(
                    "Failed to edit table tennis tracking message (failure %d/%d): %s",
                    consecutive_edit_failures, MAX_CONSECUTIVE_MISSES, e,
                )
                if consecutive_edit_failures >= MAX_CONSECUTIVE_MISSES:
                    botlog.event(f"⚠️ Auto-stopped tracking (table tennis): match `{match_id}` message edit failed {MAX_CONSECUTIVE_MISSES}x in a row, in <#{channel_id}>")
                    await _void_leg_and_give_up("Message edit failed repeatedly")
                    break
                continue
        else:
            botlog.event(f"⚠️ Auto-stopped tracking (table tennis): match `{match_id}` never settled within {config.MAX_TRACK_HOURS}h, in <#{channel_id}>")
            await _void_leg_and_give_up("Timed out without settling")
    except asyncio.CancelledError:
        raise
    except Exception:
        log.exception("Table tennis tracker crashed unexpectedly for match %s in channel %s", match_id, channel_id)
        botlog.event(f"⚠️ Auto-stopped tracking (table tennis): match `{match_id}` crashed unexpectedly (see server logs), in <#{channel_id}>")
...
def start_tracking(
    ...
    dailylog.record_pick(
        channel_id, "tabletennistracker", key, section, label or pick_label(market, team, line), message.id,
        origin_channel_id, sport="Table Tennis", tournament=match.get("unique_tournament_name"), game_date=None,
    )
...
async def resume_all(client: discord.Client):
    ...
    """
    for key, entry in list(state.load_table_tennis().items()):
        try:
            channel_id, match_id, market, message_id = (
                entry["channel_id"], entry["match_id"], entry["market"], entry["message_id"],
            )
        except KeyError:
            log.warning("Dropping table tennis entry from an incompatible state schema: %r", entry)
            continue
        ...
        if message is None:
            botlog.event(f"⚠️ Dropped on resume (table tennis): match `{match_id}` — message/channel no longer reachable, in <#{channel_id}>")
            _forget_key(key)
            continue
        ...
        if match_slug:
            for attempt in range(MAX_CONSECUTIVE_MISSES):
                match = await asyncio.to_thread(tabletennis.get_live_update, {"slug": match_slug})
                if match:
                    break
                if attempt < MAX_CONSECUTIVE_MISSES - 1:
                    await asyncio.sleep(5)
        if not match:
            botlog.event(f"⚠️ Dropped on resume (table tennis): match `{match_id}` not found on scores24.live, in <#{channel_id}>")
            _forget_key(key)
            continue

        start_tracking(message, match, channel_id, market, owner_id, team, line)
        log.info("Resumed table tennis tracking for match %s in channel %s", match_id, channel_id)
