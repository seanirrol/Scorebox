...
- "set1_point_handicap": volleyball only - a Set 1 point-margin spread
  (e.g. "Serbia -4.5 1st Set"). Same shape as games_handicap, but against
  Set 1's own final point score (scores365.volleyball_first_set_result,
  backed by its own per-game detail call - see volleyball_set_scores)
  rather than games won across the whole match, and settles as soon as
  Set 1 itself ends rather than waiting on the whole match.
- "match_point_total"/"match_point_handicap": volleyball only - the
  match_total_games/games_handicap shape again, but against the combined
  rally-point total across every set of the WHOLE match
...
def stop_tracking(channel_id: int, game_id, market: str, team: Optional[str] = None) -> bool:
    ...
    dailylog.record_result(channel_id, "settracker", key, "void", "Manually untracked")
...
async def build_embed(
    ...
    if market == "set1_moneyline":
    ...
    elif market == "set1_point_handicap":
        breakdown = scores365.volleyball_first_set_result(game)
        decided = breakdown is not None
        if decided:
            result = scores365.grade_volleyball_set1_handicap(game, team, line)
            frozen_cols = (scores365.fmt_score(breakdown[0]), scores365.fmt_score(breakdown[1]))
        final_period_text = "1st Set Final"

    elif market == "match_point_total":
        decided = scores365.is_finished(game)
        home_points, away_points = scores365.volleyball_match_points(game)
        if status != "notstarted":
            current_total_value = home_points + away_points
        if decided:
            result = "void" if scores365.is_walkover(game) else scores365.grade_over_under(home_points + away_points, direction, line)
        ...
        home_points, away_points = scores365.volleyball_match_points(game)
        if decided:
            result = scores365.grade_volleyball_match_point_handicap(game, team, line)
        frozen_cols = (scores365.fmt_score(home_points), scores365.fmt_score(away_points))  # live-running, shown whether decided or not
...
def grade_now(game: dict, market: str, team: Optional[str], direction: Optional[str], line: Optional[float]) -> tuple[bool, Optional[str]]:
    ...
    if market == "set1_point_handicap":
        ...
        if breakdown is None:
            return False, None
        return True, scores365.grade_volleyball_set1_handicap(game, team, line)
    if market == "match_point_total":
        if not scores365.is_finished(game):
            return False, None
        if scores365.is_walkover(game):
    ...
    if market == "match_point_handicap":
        if not scores365.is_finished(game):
            return False, None
        return True, scores365.grade_volleyball_match_point_handicap(game, team, line)
    # win_a_set
...
async def _track_loop(
    ...
    async def _void_leg_and_give_up(reason: str):
        ...
        dailylog.record_result(channel_id, "settracker", key, "void", reason)
    ...
    # the rendered card is byte-identical to what's already posted.
    # Confirmed live this is what was tripping Discord's error 400009
    # (message edit attachment upload limit) and freezing cards mid-event.
    ...
    try:
        while time.monotonic() < deadline:
            ...
            if scores365.is_cancelled(game):
                # A cancelled match will never decide any of this module's
                # markets - see f5tracker.py's identical fix for why this
            ...
            if game and scores365.is_interrupted(game):
                ...
                try:
                    ...
                    log.warning("Failed to add void reaction: %s", e)
                pendingdelete.start(channel_id, message, embed.description or "")
                dailylog.record_result(channel_id, "settracker", key, "void", "Interrupted, never resumed")
                group_ids = parlaytracker.groups_for_leg(channel_id, "settracker", key)
                await parlaytracker.handle_leg_result(
                    message.channel, channel_id, message, "settracker", key, leg_label, "void", group_ids,
                )
...
async def resume_all(client: discord.Client):
    ...
    for key, entry in list(state.load_set1().items()):
        ...
        if message is None:
            ...
            merge_key = mergetracker.merged_into(channel_id, "settracker", key) if channel else None
            if merge_key:
                message = mergetracker.MergedLegPlaceholder(channel, message_id)
                log.info("Resuming merged tennis-market leg %s via merge group %s (own card intentionally gone)", key, merge_key)
            else:
