...
async def _track_loop(
    ...
    async def _void_leg_and_give_up():
        ...
        await parlaytracker.handle_leg_result(
            message.channel, channel_id, message, "soccerpropstracker", key, void_label, "void", marker_emojis,
        )
        ...
        while time.monotonic() < deadline:
            ...
            if scores365.is_finished(game):
                ...
                if direction is not None and line is not None:
                    ...
                    if result:
                        await parlaytracker.handle_leg_result(
                            message.channel, channel_id, message, "soccerpropstracker", key, leg_label, result, carry_emojis,
                        )
            ...
            if game and scores365.is_interrupted(game):
                ...
                pendingdelete.start(channel_id, message, embed.description or "")
                await parlaytracker.handle_leg_result(
                    message.channel, channel_id, message, "soccerpropstracker", key, leg_label, "void", carry_emojis,
                )
