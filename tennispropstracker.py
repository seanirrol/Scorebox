...
def prop_key(
    ...
    return f"{channel_id}:{game_id}:{competitor_id}:{stat_name}:{market}"
...
def is_tracked(
    ...
    return prop_key(channel_id, game_id, competitor_id, stat_name, direction, line) in _active
...
def list_tracked_details(channel_id: int) -> list[dict]:
    ...
    return [
        ...
        if prop_key(
            entry["channel_id"], entry["game_id"], entry["competitor_id"], entry["stat_name"],
...
def register_message(
    message_id: int, channel_id: int, game_id, competitor_id, stat_name: str, owner_id: int,
    direction: Optional[str] = None, line: Optional[float] = None,
):
    """Lets bot.py's 🗑️-reaction handler know who's allowed to delete this message."""
    _message_owners[message_id] = (channel_id, game_id, competitor_id, stat_name, direction, line, owner_id)
...
def _persist(
    ...
    data[prop_key(channel_id, game_id, competitor_id, stat_name, direction, line)] = {
...
def _forget(
    ...
    data = state.load_tennis_props()
    data.pop(prop_key(channel_id, game_id, competitor_id, stat_name, direction, line), None)
    state.save_tennis_props(data)
...
async def build_embed(
    ...
    image_bytes = await asyncio.to_thread(
        ...
        f"vs {opponent}", photo_url, player_name, stat_label, _fmt_value(current_value), color_status, period_text,
    )
...
async def _track_loop(
    ...
    async def _repost_final(embed: discord.Embed, file: discord.File):
        ...
        register_message(message.id, channel_id, game_id, competitor_id, stat_name, owner_id, direction, line)
        try:
    ...
    async def _void_leg_and_give_up(reason: str):
        ...
        await parlaytracker.handle_leg_result(
            message.channel, channel_id, message, "tennispropstracker", key, void_label, "void", group_ids,
        )
    ...
    try:
        while time.monotonic() < deadline:
            ...
            while game and scores365.map_status_type(game.get("statusGroup")) == "notstarted":
                ...
                dailylog.touch(channel_id, "tennispropstracker", key, f"NOT STARTED - <t:{int(kickoff)}:f>")
                group_ids = parlaytracker.groups_for_leg(channel_id, "tennispropstracker", key)
                if group_ids:
                    pre_home = game.get("homeCompetitor") or {}
                    pre_away = game.get("awayCompetitor") or {}
                    pre_opponent = pre_away.get("name", "?") if pre_home.get("id") == competitor_id else pre_home.get("name", "?")
                    pre_pick = (
                    ...
                    await parlaytracker.report_leg_progress(
                        message.channel, channel_id, message, "tennispropstracker", key, pre_label,
            ...
            embed, file = await build_embed(
                game, sport_id, competitor_id, player_name, stat_label, stat_name, direction, line, message_id=message.id,
            )
            leg_home = game.get("homeCompetitor") or {}
            leg_away = game.get("awayCompetitor") or {}
            leg_opponent = leg_away.get("name", "?") if leg_home.get("id") == competitor_id else leg_home.get("name", "?")
            ...
            dailylog.touch(channel_id, "tennispropstracker", key, detail)
            group_ids = parlaytracker.groups_for_leg(channel_id, "tennispropstracker", key)
            if group_ids:
                await parlaytracker.report_leg_progress(
                    message.channel, channel_id, message, "tennispropstracker", key, leg_label, detail, group_ids,
                )
            ...
            try:
                ...
                if e.status == 429 or e.code == 400009:
                    ...
                    log.warning(
                        "Failed to edit tennis prop tracking message, rate limited (failure %d/%d): %s",
                    ...
                    if consecutive_rate_limit_failures >= MAX_CONSECUTIVE_RATE_LIMIT_FAILURES:
                        botlog.event(f"⚠️ Auto-stopped tracking (tennis prop): **{player_name}** — message edit rate-limited {MAX_CONSECUTIVE_RATE_LIMIT_FAILURES}x in a row, in <#{channel_id}>")
            ...
            if game and scores365.is_interrupted(game):
                embed, file = await build_embed(
                    game, sport_id, competitor_id, player_name, stat_label, stat_name, direction, line,
...
def start_tracking(
    ...
    dailylog.record_pick(
        ...
        sport="Tennis", tournament=tournament, game_date=game_date,
    )
