...
def stop_tracking(
    ...
    dailylog.record_result(channel_id, "ufctracker", key, "void", "Manually untracked")
...
async def _track_loop(
    ...
    async def _void_leg_and_give_up(reason: str):
        ...
        dailylog.record_result(channel_id, "ufctracker", key, "void", reason)
        ...
        while time.monotonic() < deadline:
            ...
            if not refreshed:
                ...
                if consecutive_misses >= MAX_CONSECUTIVE_MISSES:
                    ...
                    await _void_leg_and_give_up("Bout not found on ESPN")
                    break
                continue
            consecutive_misses = 0
            ...
            try:
                ...
                if e.status == 429:
                    ...
                    log.warning(
                        "Failed to edit UFC tracking message, rate limited (failure %d/%d): %s",
                    ...
                    if consecutive_rate_limit_failures >= MAX_CONSECUTIVE_RATE_LIMIT_FAILURES:
                        botlog.event(f"⚠️ Auto-stopped tracking (UFC): bout `{competition_id}` message edit rate-limited {MAX_CONSECUTIVE_RATE_LIMIT_FAILURES}x in a row, in <#{channel_id}>")
...
def start_tracking(
    ...
    dailylog.record_pick(channel_id, "ufctracker", key, section, label, message.id, origin_channel_id, sport="UFC")
